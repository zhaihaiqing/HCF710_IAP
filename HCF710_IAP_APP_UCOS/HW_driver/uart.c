...
volatile unsigned char ModbusIntervalTime = Default_MODBUS_INTERVAL_TIME;//指令发送间隔时间,超时认为一条指令已接收完毕
	
volatile unsigned char USART1_INTERVAL_TIME = 0;//指令发送间隔时间,超时认为一条指令已接收完毕	
volatile unsigned char USART1_RX_Len=0;     //串口接收指令长度
volatile unsigned char USART2_INTERVAL_TIME = 0;//指令发送间隔时间,超时认为一条指令已接收完毕
volatile unsigned char USART2_RX_Len=0;     //串口接收指令长度
volatile unsigned char USART3_INTERVAL_TIME = 0;//指令发送间隔时间,超时认为一条指令已接收完毕
volatile unsigned char USART3_RX_Len=0;     //串口接收指令长度

unsigned char USART1_RX[UART1_RBUF_SIZE];
unsigned char USART2_RX[UART2_RBUF_SIZE];
unsigned char USART3_RX[UART3_RBUF_SIZE];	
...
void UART1_Configuration(unsigned int baudrate)
{
	...
	GPIO_InitStructure.GPIO_Pin =  GPIO_Pin_5;																	//RS485_RW
...
//	BSP_IntVectSet(BSP_INT_ID_USART1, USART1_IRQHandler); //设置串口1的中断向量，必须要有的
...
PUTCHAR_PROTOTYPE
{
	USART_PutChar(LOG_UART,(uint8_t) ch);
	return ch;
}
